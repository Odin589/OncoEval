...
print('Initiating PYtohn script')
...
book = xlrd.open_workbook(str(sys.argv[1]))
sheet = book.sheet_by_index(0)
results_dict = {}
for rowidx in range(sheet.nrows):
    row = sheet.row(rowidx)
    row_identifier = row[0]
    row_identifier_value = row_identifier.value
    row_results = row[1]
    row_results_value = row_results.value
    results_dict[row_identifier_value] = row_results_value
...
results = open("/Applications/MAMP/htdocs/CX3/uploads/Results.txt", "w")
results.write(str(sys.argv[1]))
print('Yuor File is being processed by the pytho script')
results.write(',')
